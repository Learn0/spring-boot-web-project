...
public class OAuth2MemberServiceImpl implements OAuth2MemberService {
	...
	public OAuth2User loadUser(OAuth2UserRequest oAuth2UserRequest) throws OAuth2AuthenticationException {
		...
		// OAuth2 로그인을 통/해 가져온 OAuth2User의 attribute를 담아주는 of 메소드.
	...
	public MemberEntity saveOrUpdate(OAuthAttributes attributes) {
		MemberEntity memberEnity = memberRepository.findByEmail(attributes.getEmail())
				.map(entity -> entity.update(attributes.getNickname(), attributes.getPhoto()))
				.orElse(attributes.toEntity());

		return memberRepository.save(memberEnity);
	}
}